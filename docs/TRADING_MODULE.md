...
**Статус:** MVP · **Эталон:** `metka-29` · **UI:** `/terminal.html`

## Desktop vs Web

Один и тот же продукт. На вебе торговля Bybit идёт через Railway (`/trade/*`), в `.app` — через IPC и ключи на диске.

| | Сайт (Vercel) | Desktop (.app) |
|---|---------------|------------------|
| Графики, алерты, синхронизация | ✅ | ✅ |
| Терминал / Вотчлист / Дневник Bybit | ✅ Railway | ✅ IPC |
...
Страница **Скрипт** и фоновый сканер паттерна **1-2** — расширение торгового слоя. На вебе и в `.app` пункт меню включается в Настройках → Системные.
...
(после ТП1 → N% лог-X, после ТП2 → BE). Уровни в **лог-шкале** (как лог-фиба):
St2 `pt4×r(X)^k`, St3 `pt2×r(Y)^k`. Одновременно работает **одна** стратегия.
