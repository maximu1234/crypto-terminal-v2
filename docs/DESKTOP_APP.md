...
3. **Первый запуск без подписи Apple** — macOS может написать *«повреждено и не может быть открыто»*. Приложение не сломано, это Gatekeeper.
   ...
   **Способ A (быстрее всего):** Терминал:
   ...
   Если всё равно *«повреждено»* — скачайте **1.0.1+** (universal .dmg): в 1.0.0 была битая подпись CI (`hardenedRuntime` без Apple ID).
...
4. Обновления: кнопка **«Обновить»** в шапке (или Multichart → «Проверить обновления…»). После **1.0.2** — корректная подпись в CI, «повреждено» не должно появляться. Окно при старте разворачивается на весь экран (maximize).
