...
**Актуально:** JWT читается из `ct_supabase_auth`; reconcile только сверяет `cloudSynced`; push — worker → REST → SDK. Мёртвые обёртки (`pushOneAlertRowQueued`, `getAuthedFresh`, …) удалены.
