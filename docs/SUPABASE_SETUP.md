...
## 5.1. Экономия лимитов Free (страница `/system`)

Для администратора: блок **«Экономия лимитов Supabase»** — чекбоксы с пояснениями (localStorage в браузере).

- Отключение Realtime, облака рисунков/флагов/алертов, автопула при фокусе, замедление фонового опроса.
- После изменения — обновить открытые вкладки Multichart (F5).
- Графики Bybit и просмотр без входа не зависят от Supabase.
